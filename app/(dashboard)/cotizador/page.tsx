...
function CotizadorContent() {
  ...
  // NUEVOS ESTADOS PARA IVA POR CATEGORÍA
  ...
  async function cargarDatosEdicion(id: string) {
    ...
    if (data) {
      ...
      // Opcional: Si guardas el estado de IVA en DB, cargarlos aquí
    }
    setLoading(false);
  }
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (evt) => {
      try {
        ...
        alert("Error al procesar el archivo Excel. Verifique el formato de las columnas.");
      } finally {
        setLoading(false);
  ...
  const subtotalMateriales = items.filter(i => i.esMaterial).reduce((acc, item) => acc + (item.cantidad * item.precio), 0);
  const subtotalMO = items.filter(i => !i.esMaterial).reduce((acc, item) => acc + (item.cantidad * item.precio), 0);
  
  const subtotal = subtotalMateriales + subtotalMO;
  const ivaMateriales = materialAfecto ? Math.round(subtotalMateriales * 0.19) : 0;
  const ivaMO = moAfecto ? Math.round(subtotalMO * 0.19) : 0;
  
  const iva = ivaMateriales + ivaMO;
  ...
  const handleGuardar = async () => {
    ...
    try {
      let result;
      if (editId || folioGenerado) {
        result = await supabase.from('cotizaciones').update(payload).eq('folio', folioGenerado || proximoFolio).select().single();
      } else {
    ...
    <div className="min-h-screen bg-[#f1f5f9] text-slate-900 pb-20 md:pb-10">
          ...
          <div className="flex items-center gap-3 w-full sm:w-auto">
            ...
            <div>
              <h1 className="text-lg md:text-xl font-black text-slate-900 leading-tight flex items-center gap-2">
                {folioGenerado || editId ? 'EDITAR' : 'NUEVA'} <span className="text-[#ffc600]">COTIZACIÓN</span>
              </h1>
        ...
        <aside className="lg:col-span-4 space-y-6">
            ...
            {!clienteSeleccionado ? (
              <div className="relative">
                ...
                {showClienteDropdown && searchCliente && (
                  <div className="absolute z-50 top-full left-0 right-0 mt-2 bg-white border border-slate-200 rounded-2xl shadow-2xl max-h-60 overflow-y-auto overflow-x-hidden">
              ...
              <div className="relative p-5 bg-slate-900 rounded-2xl text-white shadow-lg overflow-hidden border-b-4 border-[#ffc600]">
                <button onClick={() => { setClienteSeleccionado(null); setSearchCliente(''); }} className="absolute top-2 right-2 bg-white/10 hover:bg-red-500 text-white p-1.5 rounded-full transition-all">
                  <X size={14} />
                </button>
                <span className="text-[9px] font-black text-[#ffc600] uppercase tracking-widest">Registrado</span>
                ...
                <div className="mt-3 space-y-1 opacity-70">
                  <p className="text-[10px] font-bold">RUT: {clienteSeleccionado.rut}</p>
                  <p className="text-[10px] font-bold truncate">Dirección: {clienteSeleccionado.direccion}</p>
                </div>
              </div>
            )}
          </section>
          ...
          <section className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm">
            ...
            <textarea 
              ...
              placeholder="Describa el objetivo principal del servicio..." 
            />
          </section>

          {/* RESUMEN TOTALES CON CONFIGURACIÓN DE IVA POR CATEGORÍA */}
          <section className="sticky bottom-4 lg:relative lg:bottom-0 bg-slate-900 p-6 rounded-3xl text-white shadow-2xl border-b-4 border-[#ffc600]">
            ...
            <h3 className="text-3xl font-black mt-3 tracking-tighter">{formatCLP(total)}</h3>
            
            <div className="mt-6 space-y-3">
               {/* Selector IVA Materiales */}
               <div className="flex items-center justify-between p-2 rounded-xl bg-white/5 border border-white/10">
                  ...
                  <button 
                    onClick={() => setMaterialAfecto(!materialAfecto)}
                    className={`px-3 py-1 rounded-full text-[9px] font-black uppercase transition-all ${materialAfecto ? 'bg-amber-500 text-white' : 'bg-slate-700 text-slate-300'}`}
                  >
                    {materialAfecto ? 'Afecto IVA' : 'Exento'}
                  </button>
               </div>

               {/* Selector IVA Mano de Obra */}
               <div className="flex items-center justify-between p-2 rounded-xl bg-white/5 border border-white/10">
                  <div className="flex items-center gap-2">
                  ...
                  <button 
                    onClick={() => setMoAfecto(!moAfecto)}
                    className={`px-3 py-1 rounded-full text-[9px] font-black uppercase transition-all ${moAfecto ? 'bg-blue-500 text-white' : 'bg-slate-700 text-slate-300'}`}
                  >
                    {moAfecto ? 'Afecto IVA' : 'Exento'}
                  </button>
               </div>
            </div>
            ...
            <div className="mt-6 pt-6 border-t border-white/10 grid grid-cols-2 gap-4">
              <div>
                ...
                <p className="font-black text-sm">{formatCLP(subtotal)}</p>
              </div>
              <div className="text-right">
                <p className="text-[9px] font-bold text-slate-400 uppercase tracking-tight">IVA Total</p>
                <p className="font-black text-sm">{formatCLP(iva)}</p>
              </div>
            </div>
          </section>
          ...
          <section className="bg-white rounded-3xl border border-slate-200 shadow-sm flex flex-col min-h-[600px] overflow-hidden">
            <header className="p-6 md:p-8 bg-slate-50/50 border-b border-slate-100 flex flex-col sm:flex-row justify-between items-center gap-4">
              <div>
                ...
                <p className="text-[10px] font-bold text-slate-400 mt-1 uppercase tracking-tighter">Use Excel para carga masiva de materiales</p>
              </div>
              
              <div className="flex flex-wrap gap-2 w-full sm:w-auto">
                <button 
                  onClick={() => fileInputRef.current?.click()} 
                  className="flex-1 sm:flex-none bg-[#ffc600] text-slate-900 px-4 py-2.5 rounded-xl text-[10px] font-black uppercase flex items-center justify-center gap-2 hover:scale-105 transition-all shadow-sm"
                >
                  <FileUp size={14}/> Importar Excel
                </button>
                <button onClick={() => setItems([{ descripcion: '', cantidad: 1, precio: 0, esMaterial: true}, ...items])} className="flex-1 sm:flex-none bg-white border border-slate-200 text-slate-700 px-4 py-2.5 rounded-xl text-[10px] font-black uppercase flex items-center justify-center gap-2 hover:bg-slate-50 transition-all">
                  <Plus size={14}/> Material
                </button>
                <button onClick={() => setItems([{ descripcion: '', cantidad: 1, precio: 0, esMaterial: false}, ...items])} className="flex-1 sm:flex-none bg-slate-900 text-white px-4 py-2.5 rounded-xl text-[10px] font-black uppercase flex items-center justify-center gap-2 hover:bg-black transition-all">
            ...
            <div className="p-4 md:p-6 space-y-4 flex-1">
              {items.length === 0 ? (
                <div className="h-full py-32 flex flex-col items-center justify-center text-slate-300 gap-4 opacity-40">
                  <div className="p-6 bg-slate-50 rounded-full">
                    <FileText size={48} strokeWidth={1} />
                  </div>
                  <p className="text-[11px] font-black uppercase tracking-[0.2em]">No hay ítems en la cotización</p>
                </div>
              ) : (
                items.map((item, i) => (
                  <div key={i} className={`group relative flex flex-col md:flex-row gap-4 p-5 rounded-2xl border transition-all animate-in fade-in slide-in-from-left-2 ${item.esMaterial ? 'bg-amber-50/30 border-amber-100' : 'bg-white border-slate-100 hover:border-slate-300 shadow-sm'}`}>
                    ...
                    <div className={`absolute top-0 left-6 -translate-y-1/2 px-2 py-0.5 rounded text-[7px] font-black uppercase tracking-widest ${item.esMaterial ? 'bg-amber-500 text-white' : 'bg-slate-900 text-white'}`}>
                      {item.esMaterial ? 'Material' : 'Item / MO'}
                    </div>

                    <div className="flex-1">
                      <label className="text-[8px] font-black text-slate-400 uppercase mb-1.5 block ml-1 tracking-widest">Descripción de Partida</label>
                      <input 
                        className="w-full bg-white/50 md:bg-transparent p-2 md:p-0 rounded-lg text-[11px] font-black uppercase outline-none focus:text-blue-600 transition-colors" 
                        value={item.descripcion} 
                        onChange={(e) => { const n = [...items]; n[i].descripcion = e.target.value; setItems(n); }} 
                        placeholder="Ej: Montaje de tablero general 24 polos..." 
                      />
                    </div>

                    <div className="flex items-end gap-3 justify-between md:justify-end border-t md:border-t-0 pt-4 md:pt-0">
                      <div className="w-20">
                        <label className="text-[8px] font-black text-slate-400 uppercase mb-1.5 block text-center tracking-widest">Cant.</label>
                        <input 
                          type="text" 
                          className="w-full bg-slate-100 p-2 rounded-xl text-center text-[11px] font-black outline-none focus:ring-2 ring-blue-500/20" 
                          value={item.cantidad === 0 ? "" : item.cantidad} 
                          onChange={(e) => { const n = [...items]; n[i].cantidad = cleanNumber(e.target.value); setItems(n); }} 
                        />
                      </div>
                      <div className="w-32">
                        <label className="text-[8px] font-black text-slate-400 uppercase mb-1.5 block text-right tracking-widest">Precio Unit.</label>
                        <div className="relative">
                          <span className="absolute left-3 inset-y-0 flex items-center text-[10px] font-bold text-slate-400">$</span>
                          <input 
                            type="text" 
                            className="w-full bg-slate-100 pl-6 pr-3 py-2 rounded-xl text-right text-[11px] font-black outline-none focus:ring-2 ring-blue-500/20" 
                            value={item.precio === 0 ? "" : item.precio.toLocaleString('es-CL')} 
                            onChange={(e) => { const n = [...items]; n[i].precio = cleanNumber(e.target.value); setItems(n); }} 
                          />
                        </div>
                      </div>
                      <div className="w-32 hidden md:block text-right">
                        <label className="text-[8px] font-black text-slate-400 uppercase mb-1.5 block tracking-widest">Subtotal</label>
                        <p className="text-[11px] font-black text-slate-900 py-2">{formatCLP(item.cantidad * item.precio)}</p>
                      </div>
                      <button 
                        onClick={() => setItems(items.filter((_, idx) => idx !== i))} 
                        className="mb-1 p-2.5 text-slate-300 hover:text-red-500 hover:bg-red-50 rounded-xl transition-all"
                      >
                        <Trash2 size={16}/>
                      </button>
                    </div>
            ...
            <div className="p-4 md:hidden">
                <button onClick={() => setItems([{ descripcion: '', cantidad: 1, precio: 0, esMaterial: false }, ...items])} className="w-full py-4 bg-slate-50 border-2 border-dashed border-slate-200 rounded-2xl flex items-center justify-center gap-2 text-slate-400 font-bold text-xs uppercase">
                    <Plus size={16} /> Agregar nuevo ítem
                </button>
            </div>
          </section>
        </div>
      ...
      {showSuccessModal && (
        <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-md z-[200] flex items-end sm:items-center justify-center p-0 sm:p-4" onClick={() => setShowSuccessModal(false)}>
          <div className="bg-white w-full sm:max-w-md rounded-t-[3rem] sm:rounded-[3rem] overflow-hidden shadow-2xl animate-in slide-in-from-bottom duration-300" onClick={(e) => e.stopPropagation()}>
            <div className="relative p-10 text-center bg-slate-50/80 overflow-hidden">
                <div className="absolute top-0 left-0 w-full h-2 bg-[#ffc600]"></div>
                <div className="w-20 h-20 bg-green-500 text-white rounded-[2rem] flex items-center justify-center mx-auto mb-6 shadow-xl shadow-green-200 rotate-12">
                  <ShieldCheck size={40} />
                </div>
                ...
                <div className="mt-4 inline-flex items-center gap-2 bg-slate-200 px-4 py-1 rounded-full">
                  <span className="text-[10px] font-black text-slate-600 uppercase tracking-widest">{formatFolio(folioGenerado)}</span>
                </div>
            </div>
            
            <div className="p-8 space-y-4">
              <button onClick={() => descargarPDF('cliente')} disabled={loading} className="w-full flex items-center gap-4 p-5 bg-slate-900 text-white rounded-[2rem] active:scale-95 transition-all shadow-xl shadow-slate-200 hover:bg-black group">
                <div className="bg-[#ffc600] p-3 rounded-2xl text-slate-900 group-hover:rotate-12 transition-transform"><User size={24} /></div>
                <div className="text-left flex-1">
                  <p className="text-[10px] font-black uppercase text-[#ffc600] leading-none mb-1">Presupuesto Oficial</p>
                  <p className="text-sm font-black uppercase">Descargar para Cliente</p>
                </div>
                {loading ? <Loader2 className="animate-spin" size={20}/> : <Download size={20} />}
              </button>

              <button 
                onClick={() => descargarPDF('interno')} 
                disabled={loading || !items.some((i: any) => i.esMaterial)} 
                className="w-full flex items-center gap-4 p-5 bg-white border-2 border-slate-100 text-slate-900 rounded-[2rem] active:scale-95 transition-all hover:border-slate-300 disabled:opacity-40"
              >
                <div className="bg-slate-100 p-3 rounded-2xl text-slate-600"><Package size={24} /></div>
                <div className="text-left flex-1">
                  <p className="text-[10px] font-black uppercase text-slate-400 leading-none mb-1">Solo Materiales</p>
                  <p className="text-sm font-black uppercase">Listado de Compra</p>
                </div>
                {loading ? <Loader2 className="animate-spin" size={20}/> : <Download size={20} />}
              </button>

              <button onClick={() => setShowSuccessModal(false)} className="w-full py-2 text-slate-400 font-bold text-[10px] uppercase hover:text-slate-900 transition-all tracking-widest">
                Seguir Editando
              </button>
            </div>
          </div>
      ...
      <div className="lg:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-slate-200 px-6 py-3 flex justify-between items-center z-40">
        <div className="flex flex-col">
            ...
            <span className="text-lg font-black text-slate-900">{formatCLP(total)}</span>
        </div>
        <button onClick={handleGuardar} className="bg-slate-900 text-[#ffc600] px-8 py-3 rounded-2xl font-black text-[10px] uppercase shadow-lg active:scale-95 transition-all">
